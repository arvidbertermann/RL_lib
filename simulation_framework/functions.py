...
def bollinger_strategy(theta, mu, sigma, ts, num_iter=500):
    ...
    for iteration in range(num_iter):
        ...
        alpha = .1
        K = 1.
        ewma = ewma_vectorized(Xs, alpha)
        ewmvar = ewmv(Xs, alpha, ewma)
        lower_bollinger_band = ewma - K * np.sqrt(ewmvar)
        upper_bollinger_band = ewma + K * np.sqrt(ewmvar)
        ...
        for i in range(1, len(ts)):
            if Xs[i] < lower_bollinger_band[i]:
                a = 1
            elif Xs[i] > upper_bollinger_band[i]:
                a = -1
            else:
                a = 0
            position_change = a - position
            cash -= position_change * Xs[i]
            cashs.append(cash)
            position = a
            pnls.append(cash + position * Xs[i])
        pnlss.append(pnls)
...
def ewmv(data, alpha, ewma):
    result = [0.]
    for i in range(1, len(data)):
        result.append((1. - alpha) * (result[-1] + alpha * (data[i] - ewma[i-1])**2))
    return result

def random_strat(ts, Xs, num_iter=500):
    pnlss = []
    for iteration in range(num_iter):
        #if (iteration + 1) % 10 == 0: print('Iteration %d' % (iteration + 1))
        cash = 0.
        cashs = [cash]
        pnls = [cash]
        position = 0
    ...
    plt.ylabel('PnL')
    plt.title('PnL on the last iteration')
    plt.show()

    plt.plot(np.mean(pnlss, axis=0))
    plt.xlabel('Time step')
    plt.ylabel('PnL')
    plt.title('Average PnL profile across all iterations')
    plt.show()

    plt.hist([pnls[-1] for pnls in pnlss], bins=20)
    plt.xlabel('PnL')
...
def boll_plot(Xs, ts):
    ...
    lower_bollinger_band = ewma - K * np.sqrt(ewmvar)
    upper_bollinger_band = ewma + K * np.sqrt(ewmvar)
    plt.plot(ts, Xs, label='process')
    plt.plot(ts, ewma, label=r'EMA($\alpha$)')
    plt.plot(ts, ewma_vectorized(Xs, alpha / 2.), label=r'EMA($\alpha/2$)')
    plt.plot(ts, ewma_vectorized(Xs, alpha / 4.), label=r'EMA($\alpha/4$)')
    plt.plot(ts, lower_bollinger_band, label='lower Bollinger band')
    plt.plot(ts, upper_bollinger_band, label='upper Bollinger band')
    plt.legend()
    plt.show()
